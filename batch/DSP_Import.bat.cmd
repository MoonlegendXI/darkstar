...
"c:\program files\MariaDB 10.2\bin\mysqladmin" -h localhost -u root -proot DROP dspdb

ECHO Creating Database dspdb
"c:\program files\MariaDB 10.2\bin\mysqladmin" -h localhost -u root -proot CREATE dspdb

ECHO Loading dspdb tables into the database
cd E:\YggdrasilXI\darkstar\sql
FOR %%X IN (*.sql) DO ECHO Importing %%X & "c:\program files\MariaDB 10.2\bin\mysql" dspdb -h localhost -u root -proot < %%X
